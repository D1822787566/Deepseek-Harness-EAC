...
| `baitong-vision`（v4.7） | 百通视觉：视觉孪生 adapter（复用 picturereader 机制，给纯文本模型声明图片能力 → 原生缩略图 + 图片块进会话）+ `look_at_image` 按需深读；图片经常驻 opencode_v4_app（默认 `http://localhost:8102`）上传百通 CDN，Qwen3.6 语义读图 | 设置 → 百通视觉（默认启用） |
| `picturereader` | 全能读图/读文档：视觉孪生 adapter（原生缩略图 + 粘贴即用自动分析，opencode 等 pi-ai provider 也可用，杜绝 UNSUPPORTED_CONTENT）；隐私/智能/严谨三模式；本地工具链（像素扫描/3 引擎 OCR/裁剪/取色/对比/批量）；pdf/word/excel/ppt 转图片；可选外部 VLM 桥（OpenAI 兼容端点） | **默认禁用**（v4.7 起由 baitong-vision 接管），「插件 → 管理」启用 |
